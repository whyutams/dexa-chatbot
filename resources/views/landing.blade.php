...
<body class="min-h-screen font-sans text-ink">
    ...
    <script>
        ...
        function animasikanAngka(elemen) { const target = Number(elemen.dataset.target || 0); const mulai = performance.now(); const durasi = 900; function frame(waktu) { const progres = Math.min((waktu - mulai) / durasi, 1); elemen.textContent = Math.floor(progres * target).toLocaleString('id-ID'); if (progres < 1) requestAnimationFrame(frame); } requestAnimationFrame(frame); }
        document.querySelectorAll('[data-target]').forEach(animasi => animasikanAngka(animasi));
        async function catatView() { const kunci = '_ipd'; let dataLokal = null; try { dataLokal = JSON.parse(localStorage.getItem(kunci) || 'null'); } catch (error) {} try { const respons = await fetch('https://api.ipify.org/?format=json'); const data = await respons.json(); if (!data.ip || dataLokal?.ip === data.ip) return; const responsCatat = await fetch('{{ route('statistik.view') }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ ip: data.ip }) }); const hasil = await responsCatat.json(); localStorage.setItem(kunci, JSON.stringify({ ip: data.ip, date: new Date().toISOString() })); const counter = document.getElementById('jumlah-view'); if (typeof hasil.jumlah === 'number') { counter.dataset.target = hasil.jumlah; animasikanAngka(counter); } } catch (error) {} }
        catatView();
        ...
        pesan.addEventListener('click', (event) => { const tombolSaran = event.target.closest('.saran'); if (!tombolSaran || tombol.disabled) return; input.value = tombolSaran.dataset.pertanyaan; input.dispatchEvent(new Event('input')); form.requestSubmit(); });
            form.addEventListener('submit', async (event) => { event.preventDefault(); const pertanyaan = input.value.trim(); if (!pertanyaan || tombol.disabled) return; tambahPesan(pertanyaan, 'pengguna'); pesan.append(indikator); gulirKeBawah(); riwayat.push({ role: 'user', content: pertanyaan }); input.value = ''; hitung.textContent = '0'; input.style.height = 'auto'; tombol.disabled = true; indikator.classList.replace('hidden', 'flex'); gulirKeBawah(); fetch('{{ route('statistik.chat') }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': token, 'Accept': 'application/json' } }).then(r => r.json()).then(d => { const counter = document.getElementById('jumlah-chat'); if (counter && typeof d.jumlah === 'number') { counter.dataset.target = d.jumlah; animasikanAngka(counter); } }).catch(() => {}); let jawaban; const waktuMulai = Date.now(); try { const [respons] = await Promise.all([fetch('{{ route('tanya.dexa') }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ pertanyaan, riwayat: riwayat.slice(0, -1) }) }), new Promise((selesai) => setTimeout(selesai, 3000))]); const data = await respons.json(); jawaban = data.pesan || data.message || 'Maaf, pencarian belum dapat diproses.'; if (typeof data.jumlahChat === 'number' && data.jumlahChat > 0) { const counter = document.getElementById('jumlah-chat'); if (counter) { counter.dataset.target = data.jumlahChat; animasikanAngka(counter); } } } catch (error) { const sisaWaktu = Math.max(0, 3000 - (Date.now() - waktuMulai)); if (sisaWaktu) await new Promise((selesai) => setTimeout(selesai, sisaWaktu)); jawaban = 'Maaf, koneksi sedang bermasalah. Silakan coba lagi.'; } finally { riwayat.push({ role: 'assistant', content: jawaban }); tambahPesan(jawaban, 'bot'); indikator.classList.replace('flex', 'hidden'); gulirKeBawah(); tombol.disabled = false; } });
    </script>
