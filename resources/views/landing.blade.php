...
<body class="min-h-screen font-sans text-ink">
    ...
    <script>
        ...
        async function catatView() { try { let ipAddress = ''; try { const rIp = await fetch('https://api.ipify.org/?format=json'); const dIp = await rIp.json(); ipAddress = dIp.ip || ''; } catch (e) {} const responsCatat = await fetch('{{ route('statistik.view') }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify(ipAddress ? { ip: ipAddress } : {}) }); const hasil = await responsCatat.json(); const counter = document.getElementById('jumlah-view'); if (counter && typeof hasil.jumlah === 'number') { counter.dataset.target = hasil.jumlah; animasikanAngka(counter); } } catch (error) {} }
        catatView();
